...
const CoinDisplay: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {showMenu && (
        <div 
          className="absolute bottom-full left-0 mb-2 bg-green-500 rounded-xl shadow-lg py-2 min-w-48 animate-slide-up" 
          style={{ fontFamily: 'Geist Mono, cursive' }}
        >
          <div className="px-4 py-3 text-lg font-bold text-white mb-4">Use Your 💎</div>
          <button
            onClick={handleFontPickingClick}
            className="w-full px-4 py-3 text-left hover:bg-green-600 text-white flex items-center space-x-3 transition-colors rounded-lg"
          >
            <span className="text-2xl">🔤</span>
            <span className="font-bold">Pick Font</span>
            <span className="ml-auto text-sm opacity-90">5 💎</span>
          </button>
        </div>
      )}
