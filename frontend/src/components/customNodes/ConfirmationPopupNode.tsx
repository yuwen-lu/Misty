        ...
        if (selectedOptions.length > 0) {
            ...
            if (selectedOptions.length === 1 && selectedOptions[0] === "Layout") {
                convertToOutline(data.subImageScreenshot).then((outlineBase64) => {
                    ...
                    data.callOpenAI(textPrompt, outlineBase64, true, data.targetRenderCodeNodeBbox ? data.targetRenderCodeNodeBbox : defaultBoundingBox, data.renderCode); // true for json mode
                }).catch((err) => {
                    console.error(err);
                });
            } else {
                data.callOpenAI(textPrompt, data.subImageScreenshot, true, data.targetRenderCodeNodeBbox ? data.targetRenderCodeNodeBbox : defaultBoundingBox, data.renderCode); // true for json mode
            }
